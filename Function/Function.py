...
def getNumber(filepath, escape_string):
    filepath = filepath.replace('-CN.', '.').replace('-cn.', '.')
...
def getDataFromJSON(file_number, config, mode):  # 从JSON返回元数据
    ...
    if mode == 1:  # 从全部网站刮削
        ...
        # =======================================================================MIDE-139
            ...
            json_data = json.loads(javdb.main(file_number))
            print("javdb_json_data : " + str(json_data))
            if getDataState(json_data) == 0:
                json_data = json.loads(jav321.main(file_number))
            if getDataState(json_data) == 0:
        ...
        # javdb_json_data = json.loads(javdb.main(file_number))
        # print("javdb_json_data : " + str(javdb_json_data))
        # if getDataState(javdb_json_data) != 0:
        #     print(javdb_json_data['actor'])
        #     json_data['actor'] = javdb_json_data['actor']
