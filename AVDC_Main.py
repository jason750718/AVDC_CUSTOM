...
class MyMAinWindow(QMainWindow, Ui_AVDV):
    ...
    def creatFolder(self, success_folder, json_data, config):
        ...
        path = folder_name.replace('title', title).replace('studio', studio).replace('year', year).replace('runtime',
                                                                                                           runtime).replace(
            'director', director).replace('actor', json_data['file_actor']).replace('release', release).replace('number', number).replace(
    ...
    def Core_Main(self, filepath, number, mode, count):
        ...
        if re.search('^[^-]*[^ -]', filename):
            file_actor = re.findall('^[^-]*[^ -]', filename)[0]
            json_data['file_actor'] = file_actor
        # =======================================================================调试模式
        ...
        if '-cn.' in filepath or '-CN.' in filepath or '中文' in filepath or '字幕' in filepath:
            cn_sub = 1
            c_word = '-CN'  # 中文字幕影片后缀
            json_data['cn'] = 1
